...
int main(int argc, char** argv)
{
	...
	uci_set_confdir(ctx, "/usr/shellgui/shellguilighttpd/www/apps/quotas");
	struct uci_ptr ptr;

	list* quota_sections = get_all_sections_of_type(ctx, "quotas_uci", "quota");
	if(quota_sections->length > 0)
	{
		...
		while(quota_sections->length > 0)
		{
			...
			char* base_id = get_uci_option(ctx, "quotas_uci", next_quota, "id");
			char* exceeded_up_speed_str = get_uci_option(ctx, "quotas_uci", next_quota, "exceeded_up_speed");
			char* exceeded_down_speed_str = get_uci_option(ctx, "quotas_uci", next_quota, "exceeded_down_speed");
		...
		while(quota_sections->length > 0 || other_quota_section_names->length > 0)
		{
			...
			char* quota_enabled_var  = get_uci_option(ctx, "quotas_uci", next_quota, "enabled");
			int enabled = 1;
			...
			if(enabled)
			{
				char* ip = get_uci_option(ctx, "quotas_uci", next_quota, "ip");
				...
				{
					exceeded_up_speed_str = get_uci_option(ctx, "quotas_uci", next_quota, "exceeded_up_speed");
					exceeded_down_speed_str = get_uci_option(ctx, "quotas_uci", next_quota, "exceeded_down_speed");
				}
				...
				else
				{
					...
					char* quota_base_id = get_uci_option(ctx, "quotas_uci", next_quota, "id");
					if(quota_base_id == NULL)
					{
						...
						char* var_set = dynamic_strcat(4, "quotas_uci.", next_quota, ".id=", quota_base_id);
					...
					char* ignore_backup  = get_uci_option(ctx, "quotas_uci", next_quota, "ignore_backup_at_next_restore");
					int do_restore = 1;
					if(ignore_backup != NULL)
					{
						...
						if(!do_restore)
						{
							...
							char* var_name = dynamic_strcat(3, "quotas_uci.", next_quota, ".ignore_backup_at_next_restore");
					...
					char* reset_interval = get_uci_option(ctx, "quotas_uci", next_quota, "reset_interval");
					char* reset = strdup("");
					if(reset_interval != NULL)
					{
						char* reset_time     = get_uci_option(ctx, "quotas_uci", next_quota, "reset_time");
					...
					char* offpeak_hours     = get_uci_option(ctx, "quotas_uci", next_quota, "offpeak_hours");
					char* offpeak_weekdays     = get_uci_option(ctx, "quotas_uci", next_quota, "offpeak_weekdays");
					char* offpeak_weekly_ranges     = get_uci_option(ctx, "quotas_uci", next_quota, "offpeak_weekly_ranges");

					char* onpeak_hours     = get_uci_option(ctx, "quotas_uci", next_quota, "onpeak_hours");
					char* onpeak_weekdays     = get_uci_option(ctx, "quotas_uci", next_quota, "onpeak_weekdays");
					char* onpeak_weekly_ranges     = get_uci_option(ctx, "quotas_uci", next_quota, "onpeak_weekly_ranges");
					...
					for(type_index=0; type_index < 3; type_index++)
					{
						...
						char* limit = get_uci_option(ctx, "quotas_uci", next_quota, types[type_index]);
						...
						if(full_qos_active)
						{
							up_qos_mark   = get_uci_option(ctx, "quotas_uci", next_quota, "exceeded_up_class_mark");
							down_qos_mark = get_uci_option(ctx, "quotas_uci", next_quota, "exceeded_down_class_mark");
						}	
						...
						if(up_qos_mark != NULL && down_qos_mark != NULL)
						{
							...
							if(type_index == EGRESS_INDEX || type_index == INGRESS_INDEX)
							{
								...
								char* other_limit = get_uci_option(ctx, "quotas_uci", next_quota, types[other_type_index]);
								if(other_limit != NULL)
								{
	...
	if (uci_lookup_ptr(ctx, &ptr, "quotas_uci", true) == UCI_OK)
	{
