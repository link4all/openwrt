...
string_map* get_rule_definition(char* config, char* section, char* search_path);
...
int main(int argc, char **argv)
{
	...
	int run_commands	= 0;
	int usage_printed	= 0;
	while((c = getopt(argc, argv, "D:d:P:p:S:s:T:t:C:c:G:g:O:o:IiRrUu")) != -1) //section, page, css includes, javascript includes, title, output interface variables
	{
		switch(c)
		{
			case 'D':
			case 'd':
				search_path = strdup(optarg);
				break;
			case 'P':
			case 'p':
	...
	{
		string_map* def = get_rule_definition(package, section, search_path);
		if(def !=  NULL)
		{
...
string_map* get_rule_definition(char* package, char* section, char* search_path)
{
	...
	{
		uci_set_confdir(ctx, search_path);
	}
