...
class=;test_order=;srcport=;connbytes_kb=;
	eval $(echo "$env_str" | grep -E 'FORM_rule_data\['${rule_num}'\]' | cut -d '[' -f3- | tr -d ']')
	rule_num=$(expr ${rule_num} + 1)
	num_rule=$(expr ${rule_num} \* 100)

uci set qos_shellgui.download_rule_${num_rule}="download_rule"
uci set qos_shellgui.download_rule_${num_rule}.class="${class}"
uci set qos_shellgui.download_rule_${num_rule}.test_order="${test_order}"
uci set qos_shellgui.download_rule_${num_rule}.srcport="${srcport}"
uci set qos_shellgui.download_rule_${num_rule}.connbytes_kb="${connbytes_kb}"

done

uci set qos_shellgui.download.qos_monenabled="${FORM_qos_monenabled}"
uci set qos_shellgui.download.ptarget_ip="${FORM_ptarget_ip}"
uci set qos_shellgui.download.pinglimit="${FORM_pinglimit}"
uci set qos_shellgui.download.default_class="${FORM_download_default_class}"
uci set qos_shellgui.download.total_bandwidth="${POST_download_total_bandwidth}"
...
uci set qos_shellgui.upload_rule_${num_rule}="upload_rule"
uci set qos_shellgui.upload_rule_${num_rule}.class="${class}"
uci set qos_shellgui.upload_rule_${num_rule}.test_order="${test_order}"
uci set qos_shellgui.upload_rule_${num_rule}.srcport="${srcport}"
uci set qos_shellgui.upload_rule_${num_rule}.connbytes_kb="${connbytes_kb}"
