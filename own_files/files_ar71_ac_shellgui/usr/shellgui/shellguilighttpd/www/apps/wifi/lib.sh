...
[ -n "$FORM_htmode" ] && if [ ${FORM_channel} -gt 7 ]; then
...
# action: save_ap ap设置编辑
	printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
		...
		[ $? -ne 0 ] && echo '{"status": 1, "msg": "'"${_LANG_Form_SSID_length_must_more_then_2_char}"'"}' && exit 1
		if [ "${FORM_encryption}" != "none" ]; then
			[ $(expr length "$FORM_key") -gt 8 ]
			[ $? -ne 0 ] && echo '{"status": 1, "msg": "'"${_LANG_Form_Password_length_must_more_then_8_char}"'"}' && exit 1
		fi
