...
printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
		...
		[ $? -ne 0 ] && echo '{"status": 1, "msg": "'"${_LANG_Form_Lan_IP}${_LANG_Form_must_be_IP}"'"}' && exit 1	
...
EOF
    ...
    printf "Content-Type: text/html; charset=utf-8\r\n\r\n"
		...
		[ $? -ne 0 ] && echo '{"status": 1, "msg": "'"${_LANG_Form_Start_IP}${_LANG_Form_must_be_number}"'"}' && exit 1
		[ $(echo "$FORM_limit" | grep -Eo '[0-9]*') = "$FORM_limit" ]
		[ $? -ne 0 ] && echo '{"status": 1, "msg": "'"${_LANG_Form_pics}${_LANG_Form_must_be_number}"'"}' && exit 1
		[ $(echo "$FORM_leasetime" | grep -Eo '[0-9]*') = "$FORM_leasetime" ]
		[ $? -ne 0 ] && echo '{"status": 1, "msg": "'"${_LANG_Form_Lease}${_LANG_Form_must_be_number}"'"}' && exit 1
