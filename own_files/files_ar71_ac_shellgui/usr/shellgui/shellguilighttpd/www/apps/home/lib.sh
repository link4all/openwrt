...
main() {
	...
	cat <<EOF
{"status": 0, "msg": "$COOKIE_lang ${_LANG_Form_lang_has_assigned}", "jump_url": "${HTTP_REFERER}", "seconds": 2000}
EOF
	elif [ "${FORM_action}" = "change_theme" ] &>/dev/null; then
	/usr/shellgui/progs/main.sbin l_p '_LANG_Form_' ${FORM_app} $COOKIE_lang > /tmp/1
